...
//ACCESS_ID
define('OSS_ACCESS_ID', C('aliyun_oss.access_id'));
//ACCESS_KEY
define('OSS_ACCESS_KEY', C('aliyun_oss.access_key'));
...
define('OSS_AUTHOR', 'renrentou');

//加载RequestCore
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'requestcore.class.php';
//加载MimeTypes
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'mimetypes.class.php';
//语言包
...
class aliyun_oss{
	...
	public function __construct($access_id = NULL, $access_key = NULL, $hostname = NULL  ){
		...
		if(!$access_id && !defined('OSS_ACCESS_ID')){
			throw_exception(NOT_SET_OSS_ACCESS_ID);
		}

		if(!$access_key && !defined('OSS_ACCESS_KEY')){
			throw_exception(NOT_SET_OSS_ACCESS_KEY);
		}
		...
		if($access_id && $access_key){
			...
			throw_exception(NOT_SET_OSS_ACCESS_ID_AND_ACCESS_KEY);
		}

		//校验access_id&access_key 
		if(empty($this->access_id) || empty($this->access_key)){
			throw_exception(OSS_ACCESS_ID_OR_ACCESS_KEY_EMPTY);
		}

		//校验hostname
		if(NULL === $hostname){
			$this->hostname = C('aliyun_oss.host');
		}else{
	...
	public function auth($options){
		...
		if(!( ('/' == $options[self::OSS_OBJECT]) && ('' == $options[self::OSS_BUCKET]) && ('GET' == $options[self::OSS_METHOD])) && !$this->validate_bucket($options[self::OSS_BUCKET])){
			throw_exception('"'.$options[self::OSS_BUCKET].'"'.OSS_BUCKET_NAME_INVALID);
		}
	
		//验证Object
		if(isset($options[self::OSS_OBJECT]) && !$this->validate_object($options[self::OSS_OBJECT])){
			throw_exception($options[self::OSS_OBJECT].OSS_OBJECT_NAME_INVALID);
		}
		...
		if(isset($options[self::OSS_HEADERS][self::OSS_ACL]) && !empty($options[self::OSS_HEADERS][self::OSS_ACL])){
			if(!in_array(strtolower($options[self::OSS_HEADERS][self::OSS_ACL]), self::$OSS_ACL_TYPES)){
				throw_exception($options[self::OSS_HEADERS][self::OSS_ACL].':'.OSS_ACL_INVALID);
			}
		}
		...
		foreach ( $headers as $header_key => $header_value ) {
			if($header_key == 'Host') continue; //2015-06-25 liufei 解决加密url无法读取的bug
	...
	public function create_object_dir($bucket, $object, $options = NULL){
		...
		$options[self::OSS_BUCKET] = $bucket;
		$options[self::OSS_METHOD] = self::OSS_HTTP_PUT;
		$options[self::OSS_OBJECT] = trim($object,'/').'/';   //虚拟目录需要以'/结尾'
	...
	public function upload_file_by_file($bucket, $object, $file, $options = NULL){
		...
		if(!file_exists($options[self::OSS_FILE_UPLOAD])){
			return false;
			throw_exception($options[self::OSS_FILE_UPLOAD].OSS_FILE_NOT_EXIST);
		}
	...
	public function delete_objects($bucket, $objects, $options = null){
		...
		if(!is_array($objects) || !$objects){
			throw_exception('The ' . __FUNCTION__ . ' method requires the "objects" option to be set as an array.');
		}
	...
	public function upload_part($bucket, $object, $upload_id, $options = null){
		...
		if (!isset($options[self::OSS_FILE_UPLOAD]) || !isset($options['partNumber'])){
			throw_exception('The `fileUpload` and `partNumber` options are both required in ' . __FUNCTION__ . '().');
		}
	...
	public function create_mpu_object($bucket, $object, $options = null){
		...
		if(!isset($options[self::OSS_FILE_UPLOAD])){
			throw_exception('The `fileUpload` option is required in ' . __FUNCTION__ . '().');
		...
		if ($upload_position === false || !isset($upload_filesize) || $upload_filesize === false || $upload_filesize < 0){
			throw_exception('The size of `fileUpload` cannot be determined in ' . __FUNCTION__ . '().');
		}
		...
		if (isset($options['uploadId'])){
			...
			if (!$upload->isOK()){
				throw_exception('Init multi-part upload failed...');
			}
		...
		if(!$upload_part_result){
			throw_exception('any part upload failed...,pls try again');
		}
...
//		//判断是否目录
//		if(!is_dir($dir)){
//			throw_exception($dir.' is not a directory');
//		}
//		
//		$file_list_array = $this->read_dir($dir,$exclude,$recursive);
//		if(!$file_list_array){
//			throw_exception($dir.' is empty...');
//		}
//		
//		$index = 1;
//		
...
//		}
		...
		//判断是否目录
		if(!is_dir($directory)){
			throw_exception($directory.' is not a directory');
		}
				
		$object = '';
		if(isset($options['object'])){
			$object = trim($options['object'],'/');
			$object = str_replace('\\', '/', $object);
		...
		if(!$file_list_array){
			throw_exception($directory.' is empty..');
		}
		...
		foreach ($file_list_array as $item){
			...
			$file = trim(str_replace($directory, '', $item['path']),'/');
			if(!empty($object)){
				$file = trim($object,'/').'/'.$file;
			}
			$response = $this->create_mpu_object($bucket, $file, $options);
			if($response->isOK()){
				echo "{$item['path']} successful..\n";
			}else{
				echo "{$item['path']} failed..\n";
			}
		}
	}
	...
	private function log($msg){
		if(defined('ALI_LOG_PATH') ){
			...
			if(empty($log_path) || !file_exists($log_path)){
				throw_exception($log_path.OSS_LOG_PATH_NOT_EXIST);
			}
		}else{
		...
		if(!file_exists($log_path)){
			throw_exception(OSS_LOG_PATH_NOT_EXIST);
		}
		...
		if(ALI_LOG){
			if(!error_log(date('Y-m-d H:i:s')." : ".$msg."\n", 3,$log_name)){
				throw_exception(OSS_WRITE_LOG_TO_FILE_FAILED);
			}
		}
	}
	...
	private function make_object_group_xml($bucket, $object_array){
		...
		if($object_array){
			if(count($object_array) > self::OSS_MAX_OBJECT_GROUP_VALUE){
				throw_exception(OSS_OBJECT_GROUP_TOO_MANY_OBJECT);
			}
			$index = 1;
			...
			throw_exception(OSS_OBJECT_ARRAY_IS_EMPTY);
		}
	...
	private function validate_options($options){
		...
		if ($options != NULL && ! is_array ( $options )) {
			throw_exception($options.':'.OSS_OPTIONS_MUST_BE_ARRAY);
		}
	}
	...
	private function validate_content($options){
		if(isset($options[self::OSS_CONTENT])){
			if($options[self::OSS_CONTENT] == '' || !is_string($options[self::OSS_CONTENT])){
				throw_exception(OSS_INVALID_HTTP_BODY_CONTENT);
			}
		}else{
			throw_exception(OSS_NOT_SET_HTTP_CONTENT);
		}
	}
	...
	private function validate_content_length($options){
		if(isset($options[self::OSS_LENGTH]) && is_numeric($options[self::OSS_LENGTH])){
			if( ! $options[self::OSS_LENGTH] > 0){
				throw_exception(OSS_CONTENT_LENGTH_MUST_MORE_THAN_ZERO);
			}
		}else{
			throw_exception(OSS_INVALID_CONTENT_LENGTH);
		}
	}
	...
	private static function set_options_header($key, $value, &$options) {
		if (isset ( $options [self::OSS_HEADERS] )) {
			if (! is_array ( $options [self::OSS_HEADERS] )) {
				throw_exception(OSS_INVALID_OPTION_HEADERS);
			}
		} else {
			$options [self::OSS_HEADERS] = array ();
		}
