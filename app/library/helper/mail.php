...
* $smtpemailto = "xxx@163.com";//发送给谁
* $smtpuser = "xxx";//SMTP服务器的用户帐号
* $smtpusername = "XX网";//接收者显示的姓名
...
* $smtp->sendmail($smtpemailto, $smtpusermail, $smtpusername, $mailsubject, $mailbody, $mailtype);
*/
...
class helper_mail
{
	...
	var $user;
	var $pass;
	var $mailmode;//邮件服务器种类 0正常 1 Cmailserver

	/* Private Variables */
	var $sock;

	/* Constractor */
	function helper_mail($relay_host = "", $smtp_port = 25,$auth = false,$user,$pass)
	{
		$this->debug = FALSE;
	...
	function sendmail($to, $from, $fromname='', $subject = "", $body = "", $mailtype, $cc = "", $bcc = "", $additional_headers = "")
	{
		...
		$subject  = "=?".C('sys_default_charset')."?B?".base64_encode($subject)."?=";
