...
class helper_splitword
{
    ...
    {
		$this->addonDicFile = FEE_PATH.'/data/words_addons.dic';
		$this->mainDicFile = FEE_PATH.'/data/base_dic_full.dic';
     ...
     */
     function getFinallyIndex()
     {
        ...
        {			
			...
			if(strlen($v['w']) < $this->wordlen) continue;
            $w = $this->outStringEncoding($v['w']);
            if( $w == ' ' || $w == '(' || $w == ')' )
            {
                continue;
