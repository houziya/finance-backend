...
class helper_tool {
	 ...
	 * @return string
	 */
	static public function validate($verifyName = '', $length = '', $mode = '') {
		if (empty($verifyName)) $verifyName = 'default';
		if (!is_numeric($length)) $length = 4;
	 ...
	 * @return string
	 */
	static public function getValidate($verifyName = '',$expiry_time = 60) {
		if (empty($verifyName)) $verifyName = 'default';
	 ...
	 * @return boolean
	 */
	static public function checkValidate($code, $del = true, $expiry_time = '', $verifyName = '') {
		if(empty($expiry_time)) $expiry_time = 600;
		if (empty($verifyName)) $verifyName = 'default';
		$code = (string)$code;
		$sess = session('validate_'.$verifyName);
		if(empty($sess) || empty($sess['code'])) return false;
		if($del) session('validate_'.$verifyName, null);
	...
	static public function checkValidate1($code, $del = true, $expiry_time = '', $verifyName = '') {
		if(empty($expiry_time)) $expiry_time = 600;
		if (empty($verifyName)) $verifyName = 'default';
		$code = (string)$code;
		$sess = session('validate_'.$verifyName);
  ...
  /**
	...
	static function sendSign($pid='',$uid='',$isqueue = true){
		...
		if($isqueue){
			...
			$cache = cache::getInstance('redis');
			$rs = $cache->lpush('list_actions', $data);
			$res = array('status'=> $rs!==false ? 1: 0);
			return $res;
		}
        }
