...
$(function(){
       $(".content-menu>a").click(function(){
                var text = $(this).text();
                if(text == '添加配置'){
                  var url = "<?php echo url('QueryConfigAdd?vid='.$row['id'].'&sort_id='.$row[sort_id]);?>";
	...
	<div class="table-list">
		...
		<table width="100%" cellspacing="0">
			...
			<?php if(is_array($lists)): foreach($lists as $key=>$v): ?><tr>
				<td><if><input name="ids[]" id="check_<?php echo ($v["id"]); ?>" value="<?php echo ($v["id"]); ?>" type="checkbox" /></if></td>
                                <td><?php echo ($v['id']); ?></td>
				...
				<td><?php echo ($v["status_tips"]); ?></if>
                                    </td>
				<td>
				    <a href="<?php echo url('QueryConfigEdit?id='.$v['id'].'&sort_id='.$row[sort_id]);?>">修改</a>&nbsp;&nbsp;
                                    <a onclick="setStatus(2,'<?php echo ($v["id"]); ?>')" style="cursor:pointer;">设为默认</a>&nbsp;&nbsp;
				</td>
		...
		<div class="btn">
                ...
                <input type="hidden" class="button" name="vid"  id="vid"  value="<?php echo ($row['id']); ?>" />
                <input type="hidden" class="button" name="sort_id"  id="sort_id"  value="<?php echo ($row['sort_id']); ?>" />
...
<script type="text/javascript">
    $(".content-menu").prepend("<a class='on' href='<?php echo url('Index?id='.$row['sort_id'].'&menu_id=28');?>'><em>软件列表</em></a> <span>|</span><a class='on' href='<?php echo url('versionIndex?id='.$row['sort_id'].'&menu_id=30');?>'><em>版本列表</em></a> <span>|</span>");
</script>