...
(function(window,undefined){
	...
	$.removeData=function(elem,name){
		...
		if(name){
			...
			for(var n in thisCache)empty=false;if(empty)delete $.cache[id];
		}else{
			delete cache[id];
	...
	$.event={
		...
		remove:function(elem,type,callback){
			...
			if(!data)return;
			if(!type){
				for(i in data)that.remove(elem,i);return;
			};
			...
			if(cache.listeners.length===0){
				...
				for(var n in cache)empty=false;if(empty)$.removeData(elem,'@events');
			};
		...
		fix:function(event){
			...
			for(var i in event)event2[i]=event[i];return event2;
		}
	};
...
(function($,window,undefined){
	...
	artDialog.fn=artDialog.prototype={
		...
		_init:function(config){
			var that=this,DOM,icon=config.icon,iconBg=icon&&(_isIE6?{
				png:'icons/'+icon+'.png'
			}:{
		...
		content:function(msg,url){
			var prev,next,parent,display,that=this,DOM=that.DOM,wrap=DOM.wrap[0],width=wrap.offsetWidth,height=wrap.offsetHeight,left=parseInt(wrap.style.left),top=parseInt(wrap.style.top),cssWidth=wrap.style.width,$content=DOM.content,content=$content[0],iframe=null,isIframe=typeof url=='string'&&url!='';
			...
			if(typeof msg==='string'){
				...
				if(isIframe)that.data.iframe=msg.contentWindow;
			};
		...
		lock:function(){
			...
			+'px':'width:100%;height:100%',ie6Css=_isIE6?'position:absolute;left:expression('+domTxt+'.scrollLeft);top:expression('
		...
		_getDOM:function(){
			...
			var name,i=0,DOM={
				wrap:$(wrap)
			},els=wrap.getElementsByTagName('*'),elsLen=els.length;
			for(;i<elsLen;i++){
	...
	if(_skin==undefined) _skin='default';
	if(_skin){
	...
	artDialog.templates='<div class="aui_outer">'+'<table class="aui_border">'+'<tbody>'+'<tr>'+'<td class="aui_nw"></td>'+'<td class="aui_n"></td>'+'<td class="aui_ne"></td>'+'</tr>'+'<tr>'+'<td class="aui_w"></td>'+'<td class="aui_c">'+'<div class="aui_inner">'+'<table class="aui_dialog">'+'<tbody>'+'<tr>'+'<td colspan="2" class="aui_header">'+'<div class="aui_titleBar">'+'<div class="aui_title"></div>'+'<a class="aui_close" href="javascript:;">'+'\xd7'+'</a>'+'</div>'+'</td>'+'</tr>'+'<tr>'+'<td class="aui_icon">'+'<div class="aui_iconBg"></div>'+'</td>'+'<td class="aui_main">'+'<div class="aui_content"></div>'+'</td>'+'</tr>'+'<tr>'+'<td colspan="2" class="aui_footer">'+'<div class="aui_buttons"></div>'+'</td>'+'</tr>'+'</tbody>'+'</table>'+'</div>'+'</td>'+'<td class="aui_e"></td>'+'</tr>'+'<tr>'+'<td class="aui_sw"></td>'+'<td class="aui_s"></td>'+'<td class="aui_se"></td>'+'</tr>'+'</tbody>'+'</table>'+'</div>';
