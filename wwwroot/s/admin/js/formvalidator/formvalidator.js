...
(function($){
	$.formValidator={
		...
		initConfig:function(controlOptions){
			...
			//加载验证规则
			var fv_js_arr = fv_js_src.split('/');
			var fv_js_regexfile = fv_js_src.replace(fv_js_arr[fv_js_arr.length-1],'')+'formvalidator_regex.js';
			//$("<scri"+"pt>"+"</scr"+"ipt>").attr({src:fv_js_regexfile,type:'text/javascript',id:'load'}).appendTo($('head').remove('#loadScript'));
			$.ajax({async:false,type: "GET",url: fv_js_regexfile, dataType: "script",
				error :function(){ alert('请确认JS【'+fv_js_regexfile+'】是否存在'); }
			});
		},
...
var fv_js_src = document.getElementsByTagName('script')[document.getElementsByTagName('script').length - 1].src;