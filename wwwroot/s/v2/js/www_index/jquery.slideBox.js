		...
		var init = function(){
			...
			if(!settings.hideBottomBar){//ADD.JENA.201208090859
				...
				if(settings.hideClickBar){//ADD.JENA.201206300847
					tips.hover(function(){
						nums.animate({bottom: '0px'}, 'fast');
					}, function(){
						nums.animate({bottom: tips.height()+'px'}, 'fast');
					});
					nums.show().delay(2000).animate({bottom: tips.height()+'px'}, 'fast');
				}else{
					nums.show();
				}
			}
		...
		var start = function() {
			...
			ul.stop().animate(param, settings.duration*1000, settings.easing, function() {
				...
				if(active.next().size()==0){
					ul.css({bottom:0, left:0}).find('li:eq(1)').addClass('active');
