$(document).ready(function(){
	$.formValidator.initConfig({formid:"reg_form",onerror:function(msg){/*popBox(msg,'error');*/return false;},onsuccess:function(){
        if(!$("input[type='checkbox']").is(':checked')) {
            popBox('请选择注册协议后再提交','info');return false;
        }
        $.post('/register/SubYoubao',{username:$('#username').val(),mobile:$('#mobile').val(),mobilecode:$('#telcode').val(),password:$('#password').val(),password2:$('#repassword').val(),email:$('#email').val(),code:$('#verifyCode').val(),btn_submit:'register'},function(data){
            if(data.status==1) {
            	//执行回调地址
            	$("body").append(data.data);
                popBox(data.msg,'success',2);
                if(data.data != '') $("#reg-form").html(data.data);
            } else {
    ...
    //$("#email").formValidator({onshow:" ",onfocus:"邮箱不能为空",oncorrect:"正确"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,},onerror:"邮箱不能为空,请确认"})
    $("#email").formValidator({onshow:"请输入邮箱",onfocus:"邮箱不能为空",oncorrect:"该邮箱可以注册"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"邮箱两边不能有空符号"},onerror:"邮箱不能为空,请确认"})
        .functionValidator({
            fun:function(val,obj){
