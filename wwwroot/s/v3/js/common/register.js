$(document).ready(function(){
	$.formValidator.initConfig({formid:"form1",onerror:function(msg){/*popBox(msg,'error');*/return false;},onsuccess:function(){
		...
		var params = {
			...
			mobile:$('#mobile').val(),
			mobilecode:$('#telcode').val(),
			password:$('#password').val(),
			password2:$('#repassword').val(),
			code:$('#verifyCode').val(),
			xieyi:$('#xieyi_check').val(),
			btn_submit:'register'
		};
        $.post('/register/add',params,function(data){
            if(data.status==1) {
            	//执行回调地址
	...
	$("#verifyCode").formValidator({onshow:" ",onfocus:"验证码不能为空",oncorrect:" "}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"验证码不能有空符号"},onerror:"验证码不能为空,请确认"});
	$("#password").formValidator({onshow:" ",onfocus:"密码不能为空",oncorrect:" "}).inputValidator({min:6,empty:{leftempty:false,rightempty:false,emptyerror:"密码两边不能有空符号"},onerror:"密码不能为空,请确认"});
