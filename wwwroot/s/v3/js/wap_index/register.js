...
$(document).ready(function(){
	$.formValidator.initConfig({formid:"form1",onerror:function(msg){/*popBox(msg,'error');*/return false;},onsuccess:function(){
        ...
        if(!pwd) var pwd = $('#password_text').val();
        $.post('/register/add',{username:$('#username').val(),mobile:$('#mobile').val(),mobilecode:$('#telcode').val(),password:pwd,/*password2:$('#repassword').val(),*/code:$('#verifyCode').val(),xieyi:$('#xieyi_check').val(),btn_submit:'register'},function(data){
            if(data.status==1) {
            	//执行回调地址
            	$("body").append(data.data);
                popBox(data.msg,'success',2);
                reg_url = data.data2 ? data.data2 : "/index/index";
                setTimeout(function(){window.location.href=reg_url;},2000)
            } else {
    ...
    $("#mobile").formValidator({onshow:" ",onfocus:"请输入正确的手机号码",oncorrect:"该手机号可以注册"})
	...
	$("#telcode").formValidator({onshow:" ",onfocus:"手机验证码不能为空",oncorrect:" "}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"手机验证码不能有空符号"},onerror:"手机验证码不能为空,请确认"});
    ...
    //$("#password_text").formValidator({onshow:"请输入密码",onfocus:"密码不能为空",oncorrect:" "}).inputValidator({min:6,empty:{leftempty:false,rightempty:false,emptyerror:"密码两边不能有空符号"},onerror:"密码不能为空,请确认"});
...
function sendMobileCode(obj, msg_type) {
    ...
    if(!$.trim(code)){popBox('请填写验证码','info');$('#myModal3').modal('hide');return false;}
    ...
    $.post("/register/sendmobilecode",{mobile:mobile,code:code,msg_type:msg_type},function(data){
        if(data.status==1) {
            ...
            curCount = 60;
            //设置button效果，开始计时
            $(".yzm_img").html( '<a class="active">'+curCount +' S</a>');
...
function SetRemainTime() {
    if (curCount == 0) {
        ...
        $(".yzm_img a").removeAttr("disabled");//启用按钮
        $(".yzm_img a").removeClass("active");//启用按钮
        $(".yzm_img").html('<a data-toggle="modal" id="btn_yzm2" onclick="showMobileSend();">重新发送</a>');
    }
    else {
        curCount--;
        $(".yzm_img").html('<a  class="active">'+curCount +' S</a>');
...
function showMobileSend() {
    ...
    $('#myModal3').modal('show');
    setTimeout(function(){
        $('.js-pri').on('click',function(){
        sendMobileCode($('#btn_yzm2'));
        })
    },1000)
}
