 ...
 * Sqlsrv数据库驱动
 */
class db_sqlsrv extends db {

	protected $selectSql = 'SELECT T1.* FROM (SELECT feephp.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS feephp) AS T1 %LIMIT%%COMMENT%';
	...
	public function error($result = true) {
		...
		$result ? log::record($this->error, log::ERR) : throw_exception($this->error);
		return $this->error;
	}
