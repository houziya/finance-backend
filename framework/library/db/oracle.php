...
class db_oracle extends db {
	...
	private $table = '';
	protected $selectSql = 'SELECT * FROM (SELECT feephp.*, rownum AS numrow FROM (SELECT  %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%) feephp ) %LIMIT%%COMMENT%';
	...
	public function connect($config = '', $linkNum = 0) {
		if (!isset($this->linkID[$linkNum])) {
			...
			$this->connected = true;
			//注销数据库安全信息
			if (1 != C('sys_db_deploy_type')) unset($this->config);
		}
	...
	public function execute($str) {
		...
		if (preg_match("/^\s*(INSERT\s+INTO)\s+(\w+)\s+/i", $this->queryStr, $match)) {
			$this->table = C("DB_SEQUENCE_PREFIX") . str_ireplace(C("sys_db_prefix"), "", $match[2]);
	...
	public function error($result = true) {
		...
		$result ? log::record($error['message'], log::ERR) : throw_exception($error['message'], '', $error['code']);
