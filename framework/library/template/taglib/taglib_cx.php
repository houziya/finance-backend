...
{//类定义开始
    ...
    public $tags   =  array(
        ...
        'php'=>array(),
        'volist'=>array('attr'=>'from,item,key,offset,length,mod','level'=>3,'alias'=>'iterate'),
        'foreach' =>array('attr'=>'from,item,key','level'=>3),
     ...
     * volist标签解析 循环输出数据集
     * 格式：
     * <volist from="userList" item="user" empty="" >
     * {user.username}
     * {user.email}
     * </volist>
     +----------------------------------------------------------
    ...
    public function _volist($attr,$content)
    {
        ...
        // 允许使用函数设定数据集 <volist from=":fun('arg')" item="vo">{$vo.name}</volist>
        $parseStr   =  '<?php ';
        if(0===strpos($from,':')) {
            $parseStr   .= '$_result='.substr($from,1).';';
            $from   = '$_result';
        }else{
            $from   = $this->autoBuildVar($from);
        }
        $parseStr  .=  'if(is_array('.$from.')): $'.$key.' = 0;';
		if(isset($tag['length']) && '' !=$tag['length'] ) {
			$parseStr  .= ' $__LIST__ = array_slice('.$from.','.$tag['offset'].','.$tag['length'].',true);';
		}elseif(isset($tag['offset'])  && '' !=$tag['offset']){
			$parseStr  .= ' $__LIST__ = array_slice('.$from.','.$tag['offset'].',count($__LIST__),true);';
        }else{
            $parseStr .= ' $__LIST__ = '.$from.';';
        }
    ...
    public function _foreach($attr,$content)
    {
        ...
        $from= $this->autoBuildVar($from);
        $parseStr  =  '<?php if(is_array('.$from.')): foreach('.$from.' as $'.$key.'=>$'.$item.'): ?>';
        $parseStr .= $this->tpl->parse($content);
    ...
    public function _import($attr,$content,$isFile=false,$type='')
    {
        ...
        if($isFile) {
            ...
            $basepath   = !empty($tag['basepath'])?$tag['basepath']:_STATIC_;
            // 命名空间方式导入外部文件
            $array =  explode(',',$file);
            foreach ($array as $val){
                switch($type) {
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="_STATIC_/js/base.js" />
    public function _load($attr,$content)
    {
        return $this->_import($attr,$content,true);
    }

    // import别名使用 导入css文件 <css file="_STATIC_/css/Base.css" />
    public function _css($attr,$content)
    {
        return $this->_import($attr,$content,true,'css');
    }

    // import别名使用 导入js文件 <js file="_STATIC_/js/Base.js" />
