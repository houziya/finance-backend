...
class taglib
{
    ...
    public function __construct() {
        $this->tagLib  = strtolower(substr(get_class($this),7));
        $this->tpl       = getInstanceOf('template');
    }
    ...
    public function autoBuildVar($name) {
        if('FEE.' == substr($name,0,4)){
            // 特殊变量
            return $this->parseFeeVar($name);
        }elseif(strpos($name,'.')) {
            $vars = explode('.',$name);
    ...
    public function parseFeeVar($varStr){
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                case 'NOW':       $parseStr = "date('Y-m-d H:i:s',time())";break;
                case 'VERSION':  $parseStr = 'FEE_VERSION';break;
                case 'TEMPLATE':$parseStr = 'C("app_tpl_filename")';break;
                case 'LDELIM':    $parseStr = 'C("sys_tpl_l_delim")';break;
                case 'RDELIM':    $parseStr = 'C("sys_tpl_r_delim")';break;
