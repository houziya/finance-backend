...
class model {
	...
	protected $methods = array('table', 'order', 'alias', 'having', 'group', 'lock', 'distinct', 'auto', 'filter', 'validate', 'result', 'bind', 'token'); // 链操作方法列表
	...
	public function __construct($name = '', $tablePrefix = '', $connection = '') {
		...
		if (!empty($name)) {
			if (strpos($name, '.')) {
				list($this->dbName, $this->name) = explode('.', $name); //数据库名.模型名的 定义
			} else {
				$this->name = $name;
			}
		...
		if (is_null($tablePrefix)) {
			...
			$this->tablePrefix = $this->tablePrefix ? $this->tablePrefix : C('sys_db_prefix');
		}

		// 数据库初始化操作，获取数据库操作对象，当前模型有独立的数据库连接信息
		$this->db(0, empty($this->connection) ? $connection : $this->connection);
		$this->_initialize(); // 模型初始化
	}
	...
	protected function _checkTableInfo() {
		...
		if (empty($this->fields)) {
			// 如果数据表字段没有定义则自动获取
			if (C('sys_db_fields_cache')) {
				$db = $this->dbName ? $this->dbName : C('sys_db_name');
				$fields = F('_fields/' . strtolower($db . '.' . $this->name));
				if ($fields) {
					$version = C('sys_db_field_version');
	...
	public function flush() {
		...
		if (C('sys_db_field_version')) $this->fields['_version'] = C('sys_db_field_version');

		//缓存开关控制
		if (C('sys_db_fields_cache')) {
			$db = $this->dbName ? $this->dbName : C('sys_db_name');
			F('_fields/' . strtolower($db . '.' . $this->name), $this->fields); // 永久缓存数据表信息
		}
	}
	...
	public function switchModel($type, $vars = array()) {
		$class = strtolower($type) . 'model';
		if (!class_exists($class)) throw_exception($class . L('_MODEL_NOT_EXIST_'));
		// 实例化扩展模型
	...
	public function __call($method, $args) {
		if (in_array(strtolower($method), $this->methods, true)) {
			...
			// 根据某个字段获取记录
			$field = parseName(substr($method, 5));
			$where[$field] = $args[0];
			return $this->where($where)->find();
		} elseif (strtolower(substr($method, 0, 10)) == 'getfieldby') {
			// 根据某个字段获取记录的某个值
			$name = parseName(substr($method, 10));
			$where[$name] = $args[0];
	...
	protected function _parseOptions($options = array()) {
		...
		$this->options = array();
		if (!isset($options['table'])) $options['table'] = $this->getTableName(); // 自动获取表名
	...
	public function create($data = '', $type = '') {
		...
		if (isset($fields)) {
			...
			// 判断令牌验证字段
			if (C('sys_token')) $fields[] = C('sys_token_name');
			foreach ($data as $key => $val) {
    ...
    public function autoCheckToken($data) {
        ...
        if(C('sys_token')){
            ...
            // 开启TOKEN重置
            if(C('sys_token_reset')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
	...
	protected function parseSql($sql, $parse) {
		...
		if (true === $parse) {
			...
			if (is_array($parse)) {
				...
				$sql = vsprintf($sql, $parse); // SQL预处理
			}
			//if (strpos($sql, '{')) $sql = preg_replace('/\{([^}]+)\}/', $this->tablePrefix.'$1', $sql);
		}
		$this->db->setModel($this->name);
		return $sql;
	}
	...
	public function db($linkNum, $config = '', $params = array()) {
		if ('' === $linkNum && $this->db) return $this->db;
	...
	public function getModelName() {
		if (empty($this->name)) $this->name = substr(strrchr(get_class($this), '_'), 1);
		return $this->name;
	}
	...
	public function getTableName() {
		if (empty($this->trueTableName)) {
			...
			if (!empty($this->tableName)) {
				...
				$tableName .= parseName($this->name);
			}
			if (!empty($this->dbName)) $tableName = $this->dbName . '.' . $tableName;
	 ...
	 * @return Model
	 */
	public function page2($page, $listRows = null) {
	...
	/**
	 * 分页数据获取
	 * @param int $p 当前页页码，如果为空则获取$_GET分页符
	 * @return mixed
	 */
	public function page($p = '', $listRows='', $conf = array()) {
		$res = array();
		$option = $this->options;
		$count = $this->getField('count(*) AS count');
		if ($count > 0) {
			$page = isset($_GET[C('var_page')]) ? $_GET[C('var_page')] : 1;
			$p = is_numeric($p) ? $p : $page;
			$p = new helper_page($count, $p, $listRows);
			if (is_array($conf) && $conf) {
				foreach ($conf as $k => $v) {
					$k == 'setTheme' ? $p->setTheme($v) : $p->setConfig($k, $v);
				}
			}
			$option['limit'] = $p->firstRow . ',' . $p->listRows;
			$res['lists'] = $this->select($option);
			$res['pages'] = $p->show();
			$res['pages_info'] = $p->show('array');
		} else {
			$res['lists'] = array();
			$res['pages'] = '';
		}
	 ...
	 * @return mixed
	 */
	public function page_group($p = '', $listRows='', $count=0,$conf = array()) {
		$res = array();
		$option = $this->options;
		//$count = $this->getField('count(*) AS count');
		if ($count > 0) {
